...
class RRDBNet_new(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32):
        ...
        self.conv_first_2 = nn.Conv2d(in_nc, int(nf / 2), 3, stride=1, dilation=1, padding='same',
...
def default_model_path(model_type='tomo'):
    ml_path = __file__
    ml_path = '/'.join(ml_path.split('/')[:-1])
    if model_type == 'bkg':
        ml_path = ml_path + '/saved_model/tomo_denoise/tomo_denoise_RRDB4.pth'
    else: # model_type = 'tomo'
        ml_path = ml_path + '/saved_model/transmission_bkg_removal/bkg_removal_RRDB4.pth.pth'

    # print(ml_path)
    return ml_path


def load_default_model(model_type='tomo', device='cpu'):
    model = default_model(model_type)
    model_path = default_model_path()
    model.load_state_dict(torch.load(model_path, map_location=torch.device(device)))
    model.to(device)
    return model

def check_image_fitting(model, image, device='cuda'):
    ...
    if n_iter == 0:
        ...
        for i in trange(s[0]):
            ...
            for j in range(n_iter):
                p = check_image_fitting(model, p, device)
